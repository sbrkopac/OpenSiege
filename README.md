...
When launching OpenSiege, we will attempt to resolve your ```ds-install-path``` path automatically. If it cannot be resolved use the ```ds-install-path``` command line to pass the directory. This only needs to be done once as that path is written to the user config file.
